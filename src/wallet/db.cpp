...
bool CDBEnv::Open(const fs::path& pathIn, bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of minersworldcoin may be using it.\n", strPath);
        return false;
    }
...
bool CDB::VerifyEnvironment(const std::string& walletFile, const fs::path& walletDir, std::string& errorStr)
{
    ...
    boost::filesystem::path p(walletFile);
    if (walletFile != p.stem().string() + p.extension().string())
    {
